...
int main(int argc, char *argv[])
{
  ...
  TestEm3(world, particles, energy, startX, MCIndex, &scoringPerVolume, NumVolumes);
  ...
  std::cout << "World (ID " << world->id() << ") Charged-TrakL "
            << chargedTrackLength[id] / copcore::units::mm
  ...
  for (int i = 0; i < NbOfLayers; i++) {
    ...
    std::cout << std::setw(5) << i << std::setw(20) << chargedTrackLen / copcore::units::mm
              << std::setw(20) << energyDep / copcore::units::MeV << std::endl;
  }
