...
// Data structures for some simple scoring.
struct GlobalScoring {
  adept::Atomic_t<int> chargedSteps;
  adept::Atomic_t<int> hits;
  adept::Atomic_t<int> numGammas;
  adept::Atomic_t<int> numElectrons;
  adept::Atomic_t<int> numPositrons;
                            ...
                            GlobalScoring *globalScoring, ScoringPerVolume *scoringPerVolume)
{
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < activeSize; i += blockDim.x * gridDim.x) {
    ...
    auto volume         = currentTrack.current_state.Top();
    if (volume == nullptr) {
    ...
    double geometryStepLength =
        LoopNavigator::ComputeStepAndNextVolume(currentTrack.pos, currentTrack.dir, geometricalStepLengthFromPhysics,
                                                currentTrack.current_state, currentTrack.next_state);
    currentTrack.pos += (geometryStepLength + 1.e-8) * currentTrack.dir;
#endif
    ...
    atomicAdd(&globalScoring->energyDeposit, energyDeposit / MeV * copcore::units::MeV);
    atomicAdd(&scoringPerVolume->energyDeposit[volumeID], energyDeposit / MeV * copcore::units::MeV);
    ...
    if (stopped) {
      if (!isElectron) {
        ...
        // energy that the photons would have from the annihilation at rest with
        // an electron.
        atomicAdd(&globalScoring->energyDeposit, 2 * copcore::units::kElectronMassC2);
        atomicAdd(&scoringPerVolume->energyDeposit[volumeID], 2 * copcore::units::kElectronMassC2);
      }
    ...
    case 0: {
      ...
      globalScoring->secondaries++;
      globalScoring->numElectrons++;
    ...
    case 1: {
      ...
      if (transformedDeltaEkin > 2 * copcore::units::kElectronMassC2) {
        ...
        PairProduce(allTracks, manager, activeQueue, currentTrack, transformedDeltaEkin, dirSecondary);
        globalScoring->secondaries += 2;
        globalScoring->numElectrons++;
        globalScoring->numPositrons++;
      } else {
        atomicAdd(&globalScoring->energyDeposit, transformedDeltaEkin);
        atomicAdd(&scoringPerVolume->energyDeposit[volumeID], transformedDeltaEkin);
      }
    ...
    case 2: {
      ...
      globalScoring->numGammas.fetch_add(2);
      ...
      if (theGamma1Ekin > 2 * copcore::units::kElectronMassC2) {
        ...
        atomicAdd(&globalScoring->energyDeposit, theGamma1Ekin);
        atomicAdd(&scoringPerVolume->energyDeposit[volumeID], theGamma1Ekin);
      }
      if (theGamma2Ekin > 2 * copcore::units::kElectronMassC2) {
        ...
        atomicAdd(&globalScoring->energyDeposit, theGamma2Ekin);
        atomicAdd(&scoringPerVolume->energyDeposit[volumeID], theGamma2Ekin);
      }
                              ...
                              ScoringPerVolume *scoringPerVolume)
{
  ...
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < queueSize; i += blockDim.x * gridDim.x) {
    ...
    if (currentTrack.pdg == Track::pdgPositron) {
      energy += copcore::units::kElectronMassC2;
    }
...
__global__ void InitPrimaries(Track *allTracks, SlotManager *manager, adept::MParray *activeQueue, int startEvent,
                              int numEvents, double energy, double startX, const vecgeom::VPlacedVolume *world,
                              GlobalScoring *globalScoring)
{
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < numEvents; i += blockDim.x * gridDim.x) {
    ...
    track.rng_state.SetSeed(314159265 * (startEvent + i));
    track.energy       = energy;
    ...
    globalScoring->numElectrons.fetch_add(1);
  }
}

//
void TestEm3(const vecgeom::cxx::VPlacedVolume *world, int particles, double energy, double startX,
             const int *MCIndex_host, ScoringPerVolume *scoringPerVolume_host, int NumVolumes)
{
  ...
  // Allocate and initialize data structures for scoring.
  GlobalScoring *globalScoring = nullptr;
  GlobalScoring globalScoring_host;
  globalScoring_host.chargedSteps  = 0;
  globalScoring_host.hits          = 0;
  globalScoring_host.numGammas     = 0;
  globalScoring_host.numElectrons  = 0;
  globalScoring_host.numPositrons  = 0;
  globalScoring_host.secondaries   = 0;
  globalScoring_host.energyDeposit = 0;
  COPCORE_CUDA_CHECK(cudaMalloc(&globalScoring, sizeof(GlobalScoring)));
  COPCORE_CUDA_CHECK(cudaMemcpy(globalScoring, &globalScoring_host, sizeof(GlobalScoring), cudaMemcpyHostToDevice));
  ...
  COPCORE_CUDA_CHECK(cudaMemcpy(scoringPerVolume, &scoringPerVolume_devPtrs, sizeof(ScoringPerVolume), cudaMemcpyHostToDevice));

  // Rule of thumb: 500 particles per GeV priamry.
  const int numEventsConcurrently = Capacity / ((int)energy / copcore::units::GeV) / 500;
  std::cout << "INFO: Simulating " << numEventsConcurrently << " events concurrently" << std::endl;
  for (int startEvent = 1; startEvent <= particles; startEvent += numEventsConcurrently) {
    int left  = particles - startEvent + 1;
    int chunk = std::min(left, numEventsConcurrently);
    ...
    InitPrimaries<<<initBlocks, InitThreads>>>(allTracks, slotManager, currentlyActive, startEvent, chunk, energy,
                                               startX, world_dev, globalScoring);
    COPCORE_CUDA_CHECK(cudaDeviceSynchronize());
    ...
    do {
      ...
      std::cout << "iter " << std::setw(4) << iterNo << " -- tracks in flight: " << std::setw(5) << inFlight
                << " energy deposition: " << std::setw(10) << globalScoring_host.totalEnergyDeposit.load()
                << " number of secondaries: " << std::setw(5) << globalScoring_host.secondaries.load()
                << " number of hits: " << std::setw(4) << globalScoring_host.hits.load();
      std::cout << std::endl;
#endif
    ...
    if (inFlight > 0) {
      ...
      int depositBlocks = (inFlight + DepositThreads - 1) / DepositThreads;
      depositBlocks = std::min(depositBlocks, MaxBlocks);
      DepositEnergy<<<depositBlocks, DepositThreads>>>(allTracks, currentlyActive, globalScoring, scoringPerVolume);
      ClearQueue<<<1, 1>>>(currentlyActive);
    }
  ...
  // Transfer back and print global scoring.
  COPCORE_CUDA_CHECK(cudaMemcpy(&globalScoring_host, globalScoring, sizeof(GlobalScoring), cudaMemcpyDeviceToHost));
  std::cout << std::endl;
  std::cout << "Mean energy deposit          " << (globalScoring_host.energyDeposit / particles) << " GeV\n"
            << " (should be approx. equal to " << energy << " GeV of the primaries)\n"
            << "Mean number of gamma         " << ((double)globalScoring_host.numGammas.load() / particles) << "\n"
            << "Mean number of e-            " << ((double)globalScoring_host.numElectrons.load() / particles) << "\n"
            << "Mean number of e+            " << ((double)globalScoring_host.numPositrons.load() / particles) << "\n"
            << "Mean number of secondaries   " << ((double)globalScoring_host.secondaries.load() / particles) << "\n"
            << "Mean number of charged steps " << ((double)globalScoring_host.chargedSteps.load() / particles) << "\n"
            << "Mean number of hits          " << ((double)globalScoring_host.hits.load() / particles) << "\n";

  // Transfer back the scoring per volume (charged track length and energy deposit).
  COPCORE_CUDA_CHECK(
      cudaMemcpy(scoringPerVolume_host->chargedTrackLength, scoringPerVolume_devPtrs.chargedTrackLength, sizeof(double) * NumVolumes, cudaMemcpyDeviceToHost));
  COPCORE_CUDA_CHECK(
      cudaMemcpy(scoringPerVolume_host->energyDeposit, scoringPerVolume_devPtrs.energyDeposit, sizeof(double) * NumVolumes, cudaMemcpyDeviceToHost));
